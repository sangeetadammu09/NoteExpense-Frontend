...
function IncomeModal({ isIncomeModalVisible, handleIncomeCancel, sendIncomeApiResponse, transactionCategory, transactionItem }) {
  ...
  const onSubmit = (data) => {
    ...
    transactionItem == null ? addIncome(data) : updateIncome(data);
  };
  ...
  return (
    <React.Fragment>
      <Dialog open={isIncomeModalVisible} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description">
        <DialogTitle id="alert-dialog-title">
          <Grid container spacing={2}>
            <Grid size={11}>
              <Typography gutterBottom variant="h5" component="div">
                {
                 transactionItem == null ? 'Add Income' : 'Edit Income'
                }
              </Typography>
            </Grid>
            <Grid size={1}>
              <CloseIcon className="iconColor" onClick={handleIncomeCancel} />
            </Grid>
          </Grid>
        </DialogTitle>
        <DialogContent>      
        ...
        {/* <DialogActions>
            <Button onClick={handleIncomeCancel}>Disagree</Button>
          </DialogActions> */}
      </Dialog>
    </React.Fragment>
  );
